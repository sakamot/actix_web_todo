...
// エラーをまとめるenumを定義する
// actix_erb::ResponseError として使うためにderiveマクロでDebugを付与している必要があります
#[derive(Error, Debug)]
enum MyError {
...
// actix_web::ResponseErrorをMyErrorに実装します。
// 今回はデフォルトの実装をそのまま使いますので、新たに実装するものはありません。
...
// MyError はactix_web::ResponseErrorを実装しているので
// indexの戻り値にMyErrorを使うことができます。
#[get("/")]
